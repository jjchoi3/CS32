...
//  Homework 4
//
//  Created by Junho Choi on 2/11/21.
//
...
int Sequence<ItemType>::size() const{return m_size;}

template <typename ItemType> inline
bool Sequence<ItemType>::empty() const{return size() == 0;}
...
int Sequence<ItemType>::insert(const ItemType& value){
      // Find the Node before which to insertp
    Node* p;
    int pos;
