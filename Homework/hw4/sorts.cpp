...
//  Homework 4
//
//  Created by Junho Choi on 3/2/21.
//
...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    // Create vector of Sensor Pointers
    vector<Sensor*> ptrSensors;
    // Set each pointer to point to the corresponding Sensor in auxSensors
    for(vector<Sensor>::iterator it = auxSensors.begin();  it != auxSensors.end(); ++it){
        ptrSensors.push_back(&(*it));
    }
    // Sort vector of pointers using STL sort algorithm
    sort(ptrSensors.begin(), ptrSensors.end(), comp);
    
    // Use sorted vector of pointers to replace each sensor in sensors
