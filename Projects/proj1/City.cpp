...
void City::preachToFlatulansAroundPlayer() {
    ...
    int numFlat = 0; // records number of Flatulans
    ...
    for (int k = 0; k < m_nFlatulans; ) {
        ...
        if (rowdiff >= -1  &&  rowdiff <= 1  &&
            coldiff >= -1  &&  coldiff <= 1) {
            numFlat++; // increment num of adjacent Flatulans
            
            if(fp->possiblyGetConverted()){ // if conversion succeeds
                numFlat--; // decrement num of adjacent Flatulans
                delete m_flatulans[k];
    ...
    if(numFlat != 0) // if num of adjacent Flatuans isn't 0 after preaching, record
        m_history.record(m_player->row() - 1, m_player->col() - 1); // Record if conversion doesn't work
}
...
History& City::history(){
    return m_history; // return reference to history object
}
